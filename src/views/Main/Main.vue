...
<template>
  <a-layout style="height: 100vh">
    <template v-if="AllFlag">
      ...
      <a-drawer v-else placement="left" :closable="false" :open="open" @close="onClose"
        style="background: #001529;width: 250px">
        <Sidebar @childFun="csFun"></Sidebar>
      </a-drawer>
    </template>
    <a-layout>
      <a-layout-header :class="AllFlag === false ? 'show' : 'hied'"
        style="background: #fff; padding: 0 20px;height: 90px;line-height: 90px">
        <div>
          <div v-if="AllFlag"
            style="display:flex;justify-content:space-between;height: 50px;line-height: 50px;border-bottom: 1px solid #cccccc">
            <div>
              <menu-unfold-outlined v-if="collapsed" class="trigger" @click="showDrawer" />
          ...
          <div style="height: 40px;line-height:40px;display: flex;justify-content: space-between;align-items: center">
            ...
            <div style="display: flex;width: 90px">
              <undo-outlined
                style="height: 40px;width: 30px;display:inline-block;line-height:40px;text-align: center;border-left: 1px solid #cccccc;" />
              <dropdown />
              <expand-outlined v-if="AllFlag" @click="alterAllFlag"
                style="height: 40px;width: 30px;display:inline-block;line-height:40px;text-align: center;border-left: 1px solid #cccccc;" />
              <compress-outlined v-else @click="alterAllFlag"
                style="height: 40px;width: 30px;display:inline-block;line-height:40px;text-align: center;border-left: 1px solid #cccccc;" />
            </div>
          </div>
        </div>
      </a-layout-header>
      <a-layout-content :style="{ margin: ' 16px', background: '#fff', overflowY: 'scroll' }">
